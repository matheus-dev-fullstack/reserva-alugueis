...
class Client(models.Model):
    ...
    # Método __str__: Retorna uma representação em string do objeto do modelo. Isso é útil para quando você visualizar uma instância desse modelo no Django Admin ou em outras partes do Django. 
    def __str__(self):
        return "{} - {}".format(self.name, self.email)
    
    class Meta:
        verbose_name = 'Cliente'
        # verbose_name: Define um nome amigável para o modelo no singular. Neste caso, 'Cliente'.
        verbose_name_plural = 'Clientes'
        # verbose_name_plural: Define o nome amigável para o modelo no plural, que será usado em locais como o Django Admin. Neste caso, 'Clientes'.
        ordering = ['-id']
        # ordering: Define a ordem padrão para consultas ao banco de dados. ['-id'] significa que a consulta retornará os registros do mais recente para o mais antigo com base no campo id.
